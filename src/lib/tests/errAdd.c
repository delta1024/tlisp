...
int res;
    ...
    if (!tlisp_error_next(state, &error)) {
        res = test_fail("Could not get error from state");
        goto defer;
    }

    if (error.code != TLISP_ERR_UNTERMITATED_STRING) {
        res = test_fail("Wrong error code; expected %d, got %d",
    ...
    if (error.mlen != err_len) {
        res = test_fail("Wrong error len; expected %d, got %d", err_len, error.mlen);
        goto defer_err;
    }

    if (memcmp(message, error.message, error.mlen) != 0) {
        res = test_fail("Wrong error message; expected %s, get %.*s", message,
